...
# 判断根目录是不是有特殊字符
for i in root_path:
    if not (i.isalnum() or '\u4e00' <= i <= '\u9fa5' or i == '/' or i == '\\' or i==':'):
...
for root_file in root_files:
    ...
    for task_file in task_files:
        ...
        if not 'content' in content_info  or not 'payPermissionType' in content_info or not 'payGroupIdName' in content_info or not 'payPrice' in content_info or not 'limitFreeDays' in content_info:
            ...
            print('----\033[0;33;40m有问题的文件路径: [ %s ]\033[0m' % content_txt_path)
            continue
        else:
        ...
        for media in medias:
            ...
            if '.png'.upper() in media.upper() or '.jpg'.upper() in media.upper() or '.jpeg'.upper() in media.upper():
                ...
                image_check = imghdr.what(media_path)
                if image_check != None:
                    content_data_pics.append(media_path)
                    print('----\033[0;32;40m图片数据读取成功...\033[0m')
                else:
                    print('----\033[0;37;41m%s 不是有效的图片文件...\033[0m' % media_path)


            elif '.mp4'.upper() in media.upper() or '.mkv'.upper() in media.upper():
                print('正在检查文件: %s' % media_path)

                if '_compress' in media:
                    print('----\033[0;37;41m%s不是合法文件,请删除后重试...\033[0m' % media_path)
                    exit()
                ...
                content_data_duration = math.ceil((VideoFileClip(media_path).duration))
        ...
        if index < len(work['contents']):
            ...
            if video_path != '':
                ...
                if 3 - len(pics_path) > 0:
                    video_pics = MyVideo.getImage(video_path, video_duration, len(pics_path))
                ...
                if commpress_type == 1:
                    ...
                    compress_video = MyVideo.cutVideo(video_path, video_duration)
                    if compress_video != '':
                        video_path = compress_video
                        is_can_delete = True
                ...
                for pic_path in pics_path[:3]:    
                    print('正在检查图片文件是否损坏: %s' % pic_path)
                    pic_check = imghdr.what(pic_path)
                    if pic_check != None:
                        print('----\033[0;32;40m图片数据完整...\033[0m')
                        pic_md5 = MyUtil.getFileMd5(pic_path)
                        pic_upload = MyAws.upload(api_url, pic_path, MyUtil.getType(pic_path), accessKey, secretKey, region, bucket)
                        if pic_upload == '-1':
                            pic_upload = MyAws.upload_s3(api_url, pic_path, MyUtil.getType(pic_path), accessKey, secretKey, region, bucket)
                        if pic_upload == '-1':
                            print('\033[0;37;41m图片上传失败,无法继续执行发推,跳过处理...\033[0m')
                            continue
                        data_pics.append('public/' + pic_md5 + '.' + MyUtil.getType(pic_path))
                    else:
                ...
                # 这一步也会检查文件是否已经存在
                # 如果存在的话,则会跳过
                video_upload = MyAws.upload(api_url, video_path, MyUtil.getType(
                    video_path), accessKey, secretKey, region, bucket)
                if video_upload == '-1':
                    video_upload = MyAws.upload_s3(api_url, video_path, MyUtil.getType(
                        video_path), accessKey, secretKey, region, bucket)
                if video_upload == '-1':
            ...
            # 如果是空的,说明没有视频,那就当成图片推文去发
                ...
                for pic_path in pics_path:
                    pic_md5 = MyUtil.getFileMd5(pic_path)
                    pic_upload = MyAws.upload(api_url, pic_path, MyUtil.getType(
                        pic_path), accessKey, secretKey, region, bucket)
                    if pic_upload == '-1':
                        pic_upload = MyAws.upload_s3(api_url, pic_path, MyUtil.getType(
                            pic_path), accessKey, secretKey, region, bucket)
                    if pic_upload != '-1':
                        suss += 1
                        data_pics.append('public/' + pic_md5 + '.' + MyUtil.getType(pic_path))
            ...
            print('----\033[0;32;40m' + account + ' 的第 ' + str(index + 1) + ' 条推文处理完成\033[0m')
            print()
            print('即将处理下一条推文: ')
...
print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
print('处理完毕: 一共处理了 ' + str(contents_all) + ' 条推文,成功发推 \033[0;32m' +
      str(succ) + '\033[0m 条 失败 \033[0;31m' + str(contents_all - succ) + ' \033[0m条')
