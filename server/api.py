...
class MyApi:
    ...
    def config(api_url: str):
        ...
        while res_suss < 3:
            try:
                res = requests.get(
                    api_url + url, headers=headers, verify=False)
                res_json = res.json()
                res_suss = 3
        ...
        if res.status_code == 200:
            ...
            print('----\033[0;37;41m0m获取平台设置失败: ' +
                  res.status_code + '\033[0m')
            return ''
    ...
    def login(api_url: str, account: str, password: str, en_key: str):
        ...
        while res_suss < 3:
            try:
                res = requests.post(api_url + url, data=data,
    ...
    def loginServer(server_url: str, account: str, password: str):
        ...
        while res_suss < 3:
            try:
                res = requests.post(server_url + url, data=data,
                                    headers=headers, verify=False)
                res_json = res.json()
                res_suss = 3
            except requests.exceptions.RequestException:
    ...
    def signUp(server_url: str, token: str, account: str, password: str, areaCode: str, en_key: str):
        ...
        while res_suss < 3:
            try:
                res = requests.post(
                    server_url + url, headers=headers, data=data, verify=False)
                res_json = res.json()
                res_suss = 3
            except requests.exceptions.RequestException:
                res_suss += 1
