    ...
    def parse(url: str, path: str, letter: str, medias: list, works_path: str):
        ...
        if url[-1] == '/':
            ...
            if myrequeste.status_code == 200:
                ...
                # 解析
                obj_tiles = list(obj.xpath('//td[@class="fb-n"]/a/text()'))
                obj_urls = list(obj.xpath('//td[@class="fb-n"]/a/@href'))
                ...
                while index < len(obj_tiles):
                    ...
                    MyRequests.parse(home_url + next_url, next_path, letter,
                                     medias, works_path)
                    index += 1
        # 文件
        else:
            ...
            if not have_file and not alredy:
                ...
                print('----\033[0;32;40m文件下载完成...' + '(' +
                      time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) +
                      ')\033[0m')
                with open(os.path.join(works_path, 'video_history.txt'),
                          'a') as f:
                    f.write(url + '\n')
