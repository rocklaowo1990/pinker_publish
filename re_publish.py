...
for task in tasks:
    ...
    token = MyApi.login(
        api_url, task['data']['account'], task['data']['password'], enKey)
    if token == '' or token == '-1':
        continue
    ...
    while page_number_index <= page_number:
        ...
        task_contents.extend(MyApi.getContentList(api_url, token, page_number_index + 1, _page_size))
        page_number_index += 1
    ...
    for task_content in task_contents:
        ...
        for pay_group_id in pay_group_ids:
            if pay_group_id == task_content['works']['payPermission']['groupId']:
                content_pass = True
        ...
        publish_data['payPermissionType'] = task_content['works']['payPermission']['type']
        publish_data['payGroupId'] = pay_group_ids[0]['groupId']
        publish_data['payPrice'] = task_content['works']['payPermission']['price']
        publish_data['limitFreeDays'] = task_content['works']['payPermission']['limitFreeDays']
        publish_data['replyPermissionType'] =task_content['works']['replyPermission']['type']

        re_publish = MyApi.publish(api_url, token, publish_data, task['data']['account'])
